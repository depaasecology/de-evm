...
## Abstract[](https://docs.evmos.org/protocol/modules/evm#abstract)
...
The `x/evm` module provides this EVM familiarity on a scalable, high-throughput Proof-of-Stake blockchain. It is built as a [Cosmos SDK module](https://docs.cosmos.network/main/building-modules/intro.html) which allows for the deployment of smart contracts, interaction with the EVM state machine (state transitions), and the use of EVM tooling. It can be used on Cosmos application-specific blockchains, which alleviate the aforementioned concerns through high transaction throughput via [Tendermint Core](https://github.com/tendermint/tendermint), fast transaction finality, and horizontal scalability via [IBC](https://ibcprotocol.org/).

The `x/evm` module is part of the [ethermint library](https://pkg.go.dev/github.com/evmos/ethermint).

## Module Architecture[](https://docs.evmos.org/protocol/modules/evm#module-architecture)

> **NOTE:**: If you're not familiar with the overall module structure from the SDK modules, please check this [document](https://docs.cosmos.network/main/building-modules/structure.html) as prerequisite reading.
...
## Concepts[](https://docs.evmos.org/protocol/modules/evm#concepts)

### EVM[](https://docs.evmos.org/protocol/modules/evm#evm-1)
...
#### State[](https://docs.evmos.org/protocol/modules/evm#state)
...
#### Accounts[](https://docs.evmos.org/protocol/modules/evm#accounts)
...
#### Architecture[](https://docs.evmos.org/protocol/modules/evm#architecture)
...
#### State Transitions with Smart Contracts[](https://docs.evmos.org/protocol/modules/evm#state-transitions-with-smart-contracts)
...
#### Executing EVM bytecode[](https://docs.evmos.org/protocol/modules/evm#executing-evm-bytecode)
...
### Evmos as Geth implementation[](https://docs.evmos.org/protocol/modules/evm#evmos-as-geth-implementation)

Evmos contains an implementation of the [Ethereum protocol in Golang](https://geth.ethereum.org/docs/getting-started) (Geth) as a Cosmos SDK module. Geth includes an implementation of the EVM to compute state transitions. Have a look at the [go-ethereum source code](https://github.com/ethereum/go-ethereum/blob/master/core/vm/instructions.go) to see how the EVM opcodes are implemented. Just as Geth can be run as an Ethereum node, Evmos can be run as a node to compute state transitions with the EVM. Evmos supports Geth's standard [Ethereum JSON-RPC APIs](https://docs.evmos.org/develop/api/ethereum-json-rpc/methods) in order to be Web3 and EVM compatible.

#### JSON-RPC[](https://docs.evmos.org/protocol/modules/evm#json-rpc)
...
##### JSON-RPC Example: `eth_call`[](https://docs.evmos.org/protocol/modules/evm#json-rpc-example-eth_call)
...
### Transaction Logs[](https://docs.evmos.org/protocol/modules/evm#transaction-logs)
...
### Block Bloom[](https://docs.evmos.org/protocol/modules/evm#block-bloom)
...
## State[](https://docs.evmos.org/protocol/modules/evm#state-1)
...
### State Objects[](https://docs.evmos.org/protocol/modules/evm#state-objects)

The `x/evm` module keeps the following objects in state:

#### State[](https://docs.evmos.org/protocol/modules/evm#state-2)
...
### StateDB[](https://docs.evmos.org/protocol/modules/evm#statedb-1)
...
#### CRUD of Ethereum accounts[](https://docs.evmos.org/protocol/modules/evm#crud-of-ethereum-accounts)
...
#### EIP2930 functionality[](https://docs.evmos.org/protocol/modules/evm#eip2930-functionality)
...
#### Snapshot state and Revert functionality[](https://docs.evmos.org/protocol/modules/evm#snapshot-state-and-revert-functionality)
...
Evmos adapted the [go-ethereum journal implementation](https://github.com/ethereum/go-ethereum/blob/master/core/state/journal.go#L39) to support this, it uses a list of journal logs to record all the state modification operations done so far, snapshot is consists of a unique id and an index in the log list, and to revert to a snapshot it just undoes the journal logs after the snapshot index in reversed order.

#### Ethereum Transaction logs[](https://docs.evmos.org/protocol/modules/evm#ethereum-transaction-logs)
...
### Genesis State[](https://docs.evmos.org/protocol/modules/evm#genesis-state)
...
### Genesis Accounts[](https://docs.evmos.org/protocol/modules/evm#genesis-accounts)
...
Its main difference is that the one on Evmos uses a custom `Storage` type that uses a slice instead of maps for the evm `State` (due to non-determinism), and that it doesn't contain the private key field.
...
## State Transitions[](https://docs.evmos.org/protocol/modules/evm#state-transitions)
...
### Client-Side[](https://docs.evmos.org/protocol/modules/evm#client-side)
...
### Server-Side[](https://docs.evmos.org/protocol/modules/evm#server-side)
...
The options `authante.NewMempoolFeeDecorator()`, `authante.NewTxTimeoutHeightDecorator()` and `authante.NewValidateMemoDecorator(ak)` are the same as for a Cosmos `Tx`. Click [here](https://docs.cosmos.network/main/basics/gas-fees.html#antehandler) for more on the `anteHandler`.

#### EVM module[](https://docs.evmos.org/protocol/modules/evm#evm-module)
...
## Transactions[](https://docs.evmos.org/protocol/modules/evm#transactions)

This section defines the `sdk.Msg` concrete types that result in the state transitions defined on the previous section.

## `MsgEthereumTx`[](https://docs.evmos.org/protocol/modules/evm#msgethereumtx)
...
#### Conversion[](https://docs.evmos.org/protocol/modules/evm#conversion)
...
#### Signing[](https://docs.evmos.org/protocol/modules/evm#signing)
...
### TxData[](https://docs.evmos.org/protocol/modules/evm#txdata)
...
### `LegacyTx`[](https://docs.evmos.org/protocol/modules/evm#legacytx)
...
### `DynamicFeeTx`[](https://docs.evmos.org/protocol/modules/evm#dynamicfeetx)
...
### `AccessListTx`[](https://docs.evmos.org/protocol/modules/evm#accesslisttx)
...
## ABCI[](https://docs.evmos.org/protocol/modules/evm#abci)
...
### InitGenesis[](https://docs.evmos.org/protocol/modules/evm#initgenesis)
...
### ExportGenesis[](https://docs.evmos.org/protocol/modules/evm#exportgenesis)
...
### BeginBlock[](https://docs.evmos.org/protocol/modules/evm#beginblock)
...
### EndBlock[](https://docs.evmos.org/protocol/modules/evm#endblock)
...
## Hooks[](https://docs.evmos.org/protocol/modules/evm#hooks)
...
## `PostTxProcessing`[](https://docs.evmos.org/protocol/modules/evm#posttxprocessing)
...
### Use Case: Call Native ERC20 Module on Evmos[](https://docs.evmos.org/protocol/modules/evm#use-case-call-native-erc20-module-on-evmos)

Here is an example taken from the Evmos [erc20 module](https://docs.evmos.org/protocol/modules/erc20) that shows how the `EVMHooks` supports a contract calling a native module to convert ERC-20 Tokens into Cosmos native Coins. Following the steps from above.
...
## Events[](https://docs.evmos.org/protocol/modules/evm#events)
...
### MsgEthereumTx[](https://docs.evmos.org/protocol/modules/evm#msgethereumtx-1)
...
### ABCI[](https://docs.evmos.org/protocol/modules/evm#abci-1)

| Type        | Attribute Key | Attribute Value      |
| ----------- | ------------- | -------------------- |
| block_bloom | `"bloom"`     | `string(bloomBytes)` |

## Parameters[](https://docs.evmos.org/protocol/modules/evm#parameters)

The evm module contains the following parameters:

### Params[](https://docs.evmos.org/protocol/modules/evm#params)
...
### EVM denom[](https://docs.evmos.org/protocol/modules/evm#evm-denom)
...
For example, on Ethereum, the `evm_denom` would be `ETH`. In the case of Evmos, the default denomination is the **atto evmos**. In terms of precision, `EVMOS` and `ETH` share the same value, *i.e.* `1 EVMOS = 10^18 atto evmos` and `1 ETH = 10^18 wei`.

TIP

Note: SDK applications that want to import the EVM module as a dependency will need to set their own `evm_denom` (i.e not `"aevmos"`).

### Enable Create[](https://docs.evmos.org/protocol/modules/evm#enable-create)
...
### Enable Transfer[](https://docs.evmos.org/protocol/modules/evm#enable-transfer)
...
### Extra EIPs[](https://docs.evmos.org/protocol/modules/evm#extra-eips)
...
### Chain Config[](https://docs.evmos.org/protocol/modules/evm#chain-config)
...
#### ChainConfig Defaults[](https://docs.evmos.org/protocol/modules/evm#chainconfig-defaults)
...
## Client[](https://docs.evmos.org/protocol/modules/evm#client)

A user can query and interact with the `evm` module using the CLI, JSON-RPC, gRPC or REST.

### CLI[](https://docs.evmos.org/protocol/modules/evm#cli)

Find below a list of `evmosd` commands added with the `x/evm` module. You can obtain the full list by using the `evmosd -h` command.

#### Queries[](https://docs.evmos.org/protocol/modules/evm#queries)
...
#### Transactions[](https://docs.evmos.org/protocol/modules/evm#transactions-1)
