...
```json
 {"jsonrpc":"2.0","id":1,"result":"DE/0.1.3+/linux/go1.18"}
```
...
Creates new message call transaction or a contract creation for signed transactions. You can get signed transaction data using the [`personal_sign`](https://docs.evmos.org/develop/api/ethereum-json-rpc/methods#personal-sign) method.
...
Read about websockets in [Tendermint Events](https://docs.evmos.org/develop/api/tendermint-rpc)
...
The account can be used with [`eth_sign`](https://docs.evmos.org/develop/api/ethereum-json-rpc/methods#eth_sign) and [`eth_sendTransaction`](https://docs.evmos.org/develop/api/ethereum-json-rpc/methods#eth_sendtransaction) while it is unlocked.
...
The transaction is the same argument as for [`eth_sendTransaction`](https://docs.evmos.org/develop/api/ethereum-json-rpc/methods#eth_sendtransaction) and contains the `from` address. If the passphrase can be used to decrypt the private key belonging to `tx.from` the transaction is verified, signed and send onto the network.
...
`ecRecover` returns the address associated with the private key that was used to calculate the signature in [`personal_sign`](https://docs.evmos.org/develop/api/ethereum-json-rpc/methods#personal-sign).

#### Parameters

- Message
- Signature returned from [`personal_sign`](https://docs.evmos.org/develop/api/ethereum-json-rpc/methods#personal-sign)
